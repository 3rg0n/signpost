...
//
// Short, and the shortness is the design. Two things earn a sentence — the path, stated
// somewhere a model already reads, and when to reach for it — and anything past those is
// content signpost is asserting about somebody else's repository. A generated paragraph in
...
//
// The last sentence is triage rather than orientation, and that is why it is worth the line
// the paragraph above argues against. "Read the map" tells a model the bundle exists; it does
// not stop the same model opening the handler it would have grepped for. Naming the symptom
// that crosses modules and the page that answers it is the difference, and it asserts nothing
// about the repository — only how to use signpost's own artifact, which is signpost's to
// state. It could not ship before the data pass: a bundle whose data stores had no writers
// and no readers would have sent a reader to a page that answered nothing, and an instruction
// naming absent content spends a model's trust once (ADR 0034).
//
// The example is a data symptom because the data store page is the one page that renders
// what points *at* it, so the reverse direction a reader needs is on the page rather than
// assembled from a dozen others. A module page still reads outward.
//
...
func agentsPointer() string {
	return "## Repository map\n\n" +
		...
		"When a symptom crosses modules — the same rows written from two places, a change " +
		"that surfaced somewhere else — start from the thing involved rather than the code " +
		"you would grep first: a data store's page names every module that writes it and " +
